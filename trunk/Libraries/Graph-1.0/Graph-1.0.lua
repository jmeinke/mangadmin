...
local major, minor = "Graph-1.0", "$Revision: 40756 $"
...
function lib:DrawBar(C, sx, sy, ex, ey, color, level)
	...
	for k,v in pairs(C.GraphLib_Bars) do
		if not v:IsShown() and not Bar then
			Bar=v
			barNum=k
			Bar:Show()	
			Tri=C.GraphLib_Tris[k]
			Tri:Show()
		end
	end
	...
	Bar:SetPoint("TOPRIGHT", C, "BOTTOMLEFT", ex, MinY);

	Tri:ClearAllPoints()
	Tri:SetPoint("BOTTOMLEFT", C, "BOTTOMLEFT", sx, MinY);
	Tri:SetPoint("TOPRIGHT", C, "BOTTOMLEFT", ex, MaxY);
...
function lib:HideBars(C)
	...
	for k,v in pairs(C.GraphLib_Tris) do
		v:Hide()
	end
end
