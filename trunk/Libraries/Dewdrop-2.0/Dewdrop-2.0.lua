...
local MINOR_VERSION = "$Revision: 33343 $"
...
local WoW21 = not GetFullScreenFrame
...
elseif GetLocale() == "zhTW" then
	...
	USAGE_TOOLTIP = "用法：%s。"
	RESET_KEYBINDING_DESC = "按 Esc 鍵清除熱鍵。"
	KEY_BUTTON1 = "滑鼠左鍵"
	KEY_BUTTON2 = "滑鼠右鍵"
	DISABLED = "禁用"
...
local function AcquireLevel(self, level)
	...
	if WoW21 then
		fullscreenFrame = GetUIPanel("fullscreen")
	else
		fullscreenFrame = GetFullScreenFrame()
	end
			...
			if not hidden then
				...
				if v.type == "toggle" then
					...
					if type(v_p.get) == "function" then
						...
						if not handler then
							Dewdrop:error("Handler is required if providing a method: %q", f)
				...
				elseif v.type == "execute" then
					...
					if type(v_p.func) == "function" then
						...
						if not handler then
							Dewdrop:error("Handler is required if providing a method: %q", v_p.func)
				...
				elseif v.type == "range" then
					...
					if type(v_p.get) == "function" then
						...
						if not handler then
							Dewdrop:error("Handler is required if providing a method: %q", v_p.get)
					...
					if type(v_p.set) == "function" then
						...
						if not handler then
							Dewdrop:error("Handler is required if providing a method: %q", v_p.set)
						elseif not handler[v_p.set] then
							Dewdrop:error("Handler %q not available", v_p.set)
						end
					...
					if type(v.validate) == "table" then
						...
						if v.validate and v.validate ~= "keybinding" then
							if type(v.validate) == "function" then
								...
								if not handler then
									Dewdrop:error("Handler is required if providing a method: %q", v.validate)
	...
	elseif options.type == "text" and type(options.validate) == "table" then
		...
		for _,k in ipairs(values) do
			...
			if type(options_p.set) == "function" then
				...
				if not handler then
					Dewdrop:error("Handler is required if providing a method: %q", options_p.set)
...
function OpenSlider(self, parent)
	if not sliderFrame then
		...
		editBox:SetScript("OnEnter", sliderFrame:GetScript("OnEnter"))
		editBox:SetScript("OnLeave", sliderFrame:GetScript("OnLeave"))
